...
export function rankings(
  ...
  const window = rankWindow(period),
    now = Date.now();
  if (kind === "users") {
    const rows = all<{
      id: string;
    ...
    return {
      ...
      rows: rows
        .filter((r) => r.score > 0)
        .slice(0, 50)
        .map((r, i) => ({ ...r, rank: i + 1, premium: premiumFor(r.id) })),
      explanation:
        "Community points = 5 per new public real-venue review + 2 per first-time follower who still follows. Only verified, active accounts count. Edits and repeated follows do not add points. You can opt out in Profile.",
    };
  }
  const rows = all<{
    id: string;
  ...
  for (const row of rows)
    row.score =
      metric === "rating"
        ? (row.rating * row.reviews + 3.5 * 5) / (row.reviews + 5)
        : row.reviews;
  ...
  return {
    ...
    explanation:
      metric === "rating"
        ? "Top-rated requires at least 3 distinct reviewers this period. Score blends the average with five reference ratings of 3.5 to reduce small-sample swings."
        : "Trending counts new public reviews by verified people this period. One review per person per spot; sample venues, hidden reviews, and edits do not add activity.",
  };
}
