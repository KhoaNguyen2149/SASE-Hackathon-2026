...
export function migrateSocial(db: DatabaseSync) {
  ...
  try {
    ...
    db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS rewards(user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,reward_key TEXT NOT NULL,entity_id TEXT NOT NULL,kind TEXT NOT NULL,xp INTEGER NOT NULL,coins INTEGER NOT NULL,local_date TEXT NOT NULL,created_at INTEGER NOT NULL,minutes INTEGER NOT NULL DEFAULT 0,PRIMARY KEY(user_id,reward_key));
      CREATE TABLE IF NOT EXISTS cosmetics_owned(user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,cosmetic_id TEXT NOT NULL,cost INTEGER NOT NULL,created_at INTEGER NOT NULL,PRIMARY KEY(user_id,cosmetic_id));
      CREATE TABLE IF NOT EXISTS subscriptions(user_id TEXT PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,customer_id TEXT UNIQUE,subscription_id TEXT UNIQUE,status TEXT NOT NULL DEFAULT 'inactive',period_end INTEGER NOT NULL DEFAULT 0,updated_at INTEGER NOT NULL);
      CREATE TABLE IF NOT EXISTS billing_events(id TEXT PRIMARY KEY,created_at INTEGER NOT NULL);
      CREATE TABLE IF NOT EXISTS ai_usage(user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,day TEXT NOT NULL,calls INTEGER NOT NULL DEFAULT 0,PRIMARY KEY(user_id,day));
      CREATE UNIQUE INDEX IF NOT EXISTS users_firebase ON users(firebase_uid) WHERE firebase_uid IS NOT NULL;
